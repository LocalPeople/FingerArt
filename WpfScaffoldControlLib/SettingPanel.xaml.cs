...
namespace XcWpfControlLib.WpfScaffoldControlLib
{
    ...
    public partial class SettingPanel : UserControl
    {
        ...
        public Dictionary<string, string> GetStructuralProperties()
        {
            ...
            result.Add(_viewModel[0].ShortName, _viewModel[0].Value);
            result.Add(_viewModel[1].ShortName, _viewModel[1].Value);
            result.Add(_viewModel[2].ShortName, _viewModel[2].Value);
            result.Add(_viewModel[4].ShortName, _viewModel[4].Value);
            result.Add(_viewModel[5].ShortName, _viewModel[5].Value);
            result.Add(_viewModel[22].ShortName, _viewModel[22].Value);
            return result;
        }
        #endregion
    }
    ...
    public class SettingViewModel : INotifyPropertyChanged
    {
        ...
        public static SettingViewModel[] GetAllSettingProperties(PropertyChangedEventHandler handler = null)
        {
            ...
            {
                new SettingViewModel("LGZJ","立杆纵距(m)",1.5){ PropertyChanged=handler,GroupName="布置参数"},
                new SettingViewModel("LGHJ","立杆横距(m)",0.8){ PropertyChanged=handler,GroupName="布置参数"},
                new SettingViewModel("BJ","步距(m)",1.8){ PropertyChanged=handler,GroupName="布置参数"},
                new SettingViewModel("LQJBS","连墙件步数",2){ PropertyChanged=handler,GroupName="布置参数"},
                new SettingViewModel("LQJKS","连墙件跨数",2){ PropertyChanged=handler,GroupName="布置参数"},
                new SettingViewModel("NJJQ","内排架距墙长度(mm)",300){ PropertyChanged=handler,GroupName="布置参数"},
                new SettingViewModel("ZJXS","扣件抗滑承载力系数",0.8){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JSJYT","脚手架用途","结构脚手架"){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("SGHZ","施工荷载均布参数(kN/m2)",3){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("SGCS","同时施工层数",2){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JBFY","基本风压(kN/m2)",0.3){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("LGFY","立杆稳定性时风压高度变化系数",0.65){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("LQJFY","连墙件强度时风压高度变化系数",0.65){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("TXXS","风荷载体型系数",1.3){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("LGZZ","每米立杆数承受的结构自重(kN)",0.1295){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JSBLB","脚手板类别","冲压钢脚手架"){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JSBZZ","脚手板自重标准值(kN/m2)",0.3){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("DJBLB","栏杆、挡板类别","栏杆、冲压钢挡脚板"){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("DJBZZ","栏杆、挡脚板自重标准值(kN/m2)",0.16){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("AQWZZ","安全设施与安全网自重标准值(kN/m2)",0.005){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("DJTLX","地基土类型","砂土"){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("DJCZL","地基承载力标准值(kPa)",140){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JCMJ","基础底面扩展面积(m2)",0.09){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JCXS","基础降低系数",1){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("JSCDXS","计算长度系数",1.5){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("LGWDXS","轴心受压立杆的稳定系数",0.188){ PropertyChanged=handler,GroupName="荷载参数"},
                new SettingViewModel("1DSGD","该分段顶部高度(m)",14.4){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("XGXH","型钢型号",14){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("HNTBH","楼板混凝土标号","C40"){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("JMMJ","型钢截面面积(cm2)",21.5){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("LLZL","型钢理论质量(kg/m)",16.9){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("ZDBJ","转动半径ix(cm)",5.75){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("JMML","截面模量Wx(cm3)",102){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("ZDGL","转动惯量Ix(cm4)",712){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("1LQJFY","本段顶部风压高度变化系数",0.65){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("MGXTBZ","锚固段长度与悬挑段长度比值",1.3){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("HNTKLQD","混凝土抗拉强度(N/mm2)",1.71){ PropertyChanged=handler,GroupName="悬挑参数"},
                new SettingViewModel("HNTKYQD","混凝土抗压强度(N/mm2)",19.2){ PropertyChanged=handler,GroupName="悬挑参数"},
            };
        }
